...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('voir-classement')
    .setDescription('Afficher le classement du serveur dans ce salon'),
  ...
  async execute(interaction, client) {
    ...
    for (let i = 0; i < top.length; i++) {
      ...
      try { const m = await interaction.guild.members.fetch(u.user_id); name = m.displayName; } catch {}
    ...
    const myRank = allUsers.findIndex(u => u.user_id === interaction.user.id) + 1;
